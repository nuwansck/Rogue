...
| 1.4 | 2026-04-28 | Hotfix — daily report crash. `send_daily_report` was passing a `session_start_sgt` keyword arg that `msg_daily_report` doesn't accept, causing the daily Telegram report to throw `TypeError` every night since Apr 22. Fix: removed the unused arg from the call site in `reporting.py:371` plus the dead `_rep_settings` / `_sess_hour` helper lines that produced it. No other behavior changes — bot trading was unaffected; only the daily wrap-up Telegram message was failing to send. |
